	...
	static void showStatus() {
		...
		switch (status) {
			...
			case failedArrayIndexOutOfBounds:
				System.out.println("Program has failed due to an Array Index that is out of bounds.");
				break;
		}
		if (status != halted) {
			dump();
	...
	static void callPrimitive(int primitiveDisplacement) {
		...
		switch (primitiveDisplacement) {
			...
			case Machine.disposeDisplacement:
				...
				break;
			case Machine.idxchkDisplacement:
				// Get the upper bound.
				// Get the lower bound.
				// Finally check the index to check.
				if (data[ST - 1] <= data[ST - 3] || data[ST - 2] > data[ST - 3]) {
					status = failedArrayIndexOutOfBounds;
				} else {
					ST = ST - 3;
				}
				break;
